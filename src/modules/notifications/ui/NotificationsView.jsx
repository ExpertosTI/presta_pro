...
const NOTIFICATION_COLORS = {
    ...
    LOCATION_ALERT: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
};
...
const TYPE_LABELS = {
    ...
    OVERDUE: 'Mora',
    LOCATION_ALERT: 'Alertas de Ubicación'
};
        ...
        <div className="space-y-4 sm:space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">Notificaciones</h2>
                ...
                {unreadCount > 0 && (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors touch-manipulation min-h-[44px] text-sm font-medium"
                    >
                        <CheckCheck size={16} />
                        Marcar todas
                    </button>
            ...
            <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700 overflow-x-auto scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0">
                {[
                    { id: 'all', label: 'Todas', icon: Bell },
                    { id: 'unread', label: 'Sin leer', icon: AlertTriangle, count: unreadCount },
                    { id: 'preferences', label: 'Config', icon: Settings }
                ].map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={`flex items-center gap-1.5 px-3 sm:px-4 py-3 font-medium transition-colors border-b-2 whitespace-nowrap text-sm touch-manipulation min-h-[44px] ${activeTab === tab.id
                    ...
                    >
                        ...
                        {tab.label}
                        {tab.count > 0 && (
                            <span className="px-1.5 py-0.5 bg-red-500 text-white text-[10px] rounded-full">{tab.count}</span>
                        )}
                    </button>
                ))}
            </div>
...
function NotificationCard({ notification, expandedId, handleNotificationClick, handleWhatsAppReminder, handleDelete }) {
    ...
    return (
        <div
            ...
            className={`p-3 sm:p-4 rounded-xl border transition-all cursor-pointer active:scale-[0.98] touch-manipulation ${notification.read
        ...
        >
            <div className="flex items-start gap-2 sm:gap-3">
                {/* Icon */}
                <div className={`p-1.5 sm:p-2 rounded-lg shrink-0 ${NOTIFICATION_COLORS[notification.type] || NOTIFICATION_COLORS.SYSTEM}`}>
                    <span className="text-lg sm:text-xl">{NOTIFICATION_ICONS[notification.type] || '📌'}</span>
                </div>

                {/* Content */}
                <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                        <h4 className={`text-sm font-semibold leading-tight ${notification.read ? 'text-slate-700 dark:text-slate-300' : 'text-slate-900 dark:text-slate-100'}`}>
                            {notification.title}
                        </h4>
                    ...
                    <p className={`text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-1 ${isExpanded ? '' : 'line-clamp-2'}`}>
                        {notification.message}
                    </p>
                        ...
                        <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
                            {notification.actionUrl && (
                                <a
                                    ...
                                    className="flex-1 py-2.5 px-3 bg-blue-600 text-white text-sm font-medium rounded-lg text-center active:bg-blue-700 touch-manipulation min-h-[44px] flex items-center justify-center"
                                >
                                    Abrir →
                                </a>
                            )}
                            <button
                                ...
                                className="py-2.5 px-3 bg-green-500 text-white text-sm font-medium rounded-lg flex items-center gap-1 active:bg-green-600 touch-manipulation min-h-[44px]"
                            >
                            ...
                            <button
                                ...
                                className="py-2.5 px-3 bg-red-100 text-red-600 text-sm font-medium rounded-lg active:bg-red-200 touch-manipulation min-h-[44px] flex items-center justify-center"
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
